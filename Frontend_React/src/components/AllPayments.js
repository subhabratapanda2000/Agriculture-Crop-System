...
const AllPayments = () => {
  ...
  const renderTooltip = (props) => (
    <Tooltip id="button-tooltip" {...props}>
      Click Here to view Payment History of this user
    </Tooltip>
  );
    ...
    service
      ...
      .then((response) => {
        ...
        if (response.data.length == 0) {
          setContent(false);
          toast.info("There have not any Payments.");
        }
      })
      .catch((error) => {
        setLoading(false);
  ...
  const searchPayment = () => {
    if (findCid.trim() !== "") {
      ...
      service
        ...
        .then(() => {
          history.push(`/customerPayment/${findCid}`);
        })
        .catch(() => {
          setLoading(false);
          setContent(true);
  ...
  const handleHistory = (id) => {
    history.push(`/customerPayment/${id}`);
  };
                        ...
                        {allPayments.map((payment) => (
                          <tr key={payment.orderId}>
                            ...
                            <td>
                              <Link
                                onClick={() => {
                                  handleHistory(payment.customerId);
                                }}
                              >
                                <OverlayTrigger
                                  placement="right"
                                ...
                                >
                                  <i
                                    className="fa fa-money w3-xxlarge center"
