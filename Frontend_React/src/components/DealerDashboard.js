...
const DealerDashboard = () => {
  ...
  const init = () => {
    dealerService
      .findByDealerId(user.id)
      .then((dealer) => {
        console.log(dealer);
        setFid(dealer.data.fid);
        setName(dealer.data.name);
        setUsername(dealer.data.userName);
        setPassword(dealer.data.password);
        setPhone_number(dealer.data.mobileNo);
        setLocation(dealer.data.address);
        setActive(dealer.data.active);
        setJoinDate(dealer.data.joinDate);
        if (dealer.data.primeMember) {
          setPrimeMember("Yes");
        } else {
          setPrimeMember("No");
        }
        setRole(dealer.data.role);
        setLoading(false);
        setContent(true);
      })
      .catch((error) => {
              ...
              <div className="w3-twothird">
                ...
                <table className="w3-table-all w3-striped w3-hoverable w3-white">
                  <tbody>
                    ...
                    <tr>
                      <td>
                        <Link className="btn btn-info" to={`/updateDealer`}>
                          Update
                        </Link>
                      </td>
                    </tr>
